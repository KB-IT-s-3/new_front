      ...
      <table>
        ...
        <tbody>
          <tr v-for="(item, index) in filteredItems" :key="index">
            ...
            <td><input type="date" v-model="item.date" @change="updateItem(item)"></td>
            <td><input type="text" v-model="item.content" @change="updateItem(item)"></td>
            <td><input type="number" v-model="item.amount" @change="updateItem(item)"></td>
            <td>
              <select v-model="item.deposit" @change="updateItem(item)">
                <option :value="true">수입</option>
                <option :value="false">지출</option>
              </select>
            </td>
            <td><input type="text" v-model="item.category" @change="updateItem(item)"></td>
          </tr>
        </tbody>
      </table>
      <div class="totals">
  ...
  export default {
    ...
    methods: {
      ...
      async updateItem(item) {
        await axios.put(`http://localhost:3000/${this.userId}`, { items: this.items });
      },
      async deleteItems() {
        // 선택된 항목만 삭제
